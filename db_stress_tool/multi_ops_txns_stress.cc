...
Status MultiOpsTxnsStressTest::UpdatePrimaryIndexValueTxn(ThreadState* thread,
  ...
  fprintf(stdout, "Update b txn %s, (%d, %d, %d) -> (%d, %d, %d)\n", txn->GetName().c_str(), (int)a, (int)b-b_delta, (int)c, (int)a, (int)b, (int)c);
  fflush(stdout);
  s = WriteToCommitTimeWriteBatch(*txn);
  if (!s.ok()) {
    return s;
...
void MultiOpsTxnsStressTest::ScanExistingDb(SharedState* shared, int threads) {
  ...
  std::vector<KeySet> non_existing_c_uniqs(threads);
  {
    ...
    for (int i = 0; i < threads; ++i) {
      ...
      // space remain the same across different runs. Will need to relax. // TODO: does transactions not modify this?
      if (existing_a_uniqs[i].size() != high - low - 1) {
        fprintf(stderr, "existing_a_uniqs[%d] size is %d, non_existing size is %d\n", i, (int)(existing_a_uniqs[i].size()), (int)(non_existing_a_uniqs[i].size()));
        fprintf(stderr, "existing_c_uniqs[%d] size is %d, non_existing size is %d\n", i, (int)(existing_c_uniqs[i].size()), (int)(non_existing_c_uniqs[i].size()));
