...
Status CompactionJob::Run() {
  ...
  for (const auto& state : compact_->sub_compact_states) {
    for (const auto& output : state.GetOutputs()) {
      total_output_num += output.table_properties->num_entries - output.table_properties->num_range_deletions;
    }
  }
    ...
    // Print the expected number of records from stats and penultimate_level_stats
    fprintf(stdout, "Expected output records: %" PRIu64 " (main level: %" PRIu64 ", penultimate level: %" PRIu64 ")\n",
            expected, 
            compaction_stats_.stats.num_output_records,
                compaction_stats_.penultimate_level_stats.num_output_records);

    if (expected != total_output_num) {
      // Print both numbers for debugging
      fprintf(stdout, "Output record count mismatch: expected %" PRIu64 ", actual %" PRIu64 "\n", 
      ...
      std::string msg =
          ...
          std::to_string(total_output_num) + " in output SST files. Compaction summary: " + scratch;
      ROCKS_LOG_WARN(
          db_options_.info_log, "[%s] [JOB %d] Compaction with status: %s",
          compact_->compaction->column_family_data()->GetName().c_str(),
          job_context_->job_id, msg.c_str());
