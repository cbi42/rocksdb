...
class MergingIterator : public InternalIterator {
 public:
  ...
  // heap, `active_` is calculated correctly (it contains the covering range
  // tombstone's level if there is one), since no range tombstone iterators are
  // skipped beyond that point iterator's current key during Seek().
