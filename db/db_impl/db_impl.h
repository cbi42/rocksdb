...
class DBImpl : public DB {
 ...
 protected:
  ...
  // @param last_seqno The latest sequence number after this WBWI is ingested.
  Status IngestWBWI(std::shared_ptr<WriteBatchWithIndex> wbwi,
                    SequenceNumber assigned_seqno, uint64_t min_prep_log,
                    const std::string& txn_name, SequenceNumber last_seqno);
 ...
 private:
  ...
  // `last_seqno` needs to be provided if `new_imm` is not nullptr. It is the
  // last_sequence number after sequence numbers are assign to new_imm. It will
  // be used for the earliest_seqno for the new live memtable.
  //
