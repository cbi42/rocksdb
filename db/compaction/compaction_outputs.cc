...
Status CompactionOutputs::AddRangeDels(
  ...
  if (!next_table_min_key.empty()) {
    assert(!comp_start_user_key ||
           icmp.user_comparator()->CompareWithoutTimestamp(
               *comp_start_user_key, upper_bound_guard) <= 0);
    ...
    if (meta.smallest.size() == 0) {
      ...
      if (lower_bound_from_sub_compact) {
        if (comp_start_user_key) {
          if (icmp.user_comparator()->CompareWithoutTimestamp(
                  *comp_start_user_key, upper_bound_guard) == 0) {
            return Status::OK();
          }
        }
      } else if (lower_bound_from_range_tombstone) {
        assert(icmp.user_comparator()->CompareWithoutTimestamp(
