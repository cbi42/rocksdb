...
struct WriteBatchIndexEntry {
  ...
  // make the entry larger than or equal to all entries with the search key so
...
class WBWIIteratorImpl final : public WBWIIterator {
 public:
  ...
  // Move to first entry >= iterate_lower_bound if provided.
  void SeekToFirst() override {
  ...
  // Move to first entry <= iterate_upper_bound if provided.
  void SeekToLast() override {
    WriteBatchIndexEntry search_entry =
  ...
  // Move to first entry within iterate bounds (if provided) that is >= key.
  ...
  // If iterator's current entry equals to `key`, then
