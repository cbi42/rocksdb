...
struct IngestExternalFileOptions {
  ...
  // no sequence number reassignment to is performed on db generated files.
  // - If multiple input files overlap with each other, for any given user key
  //   appearing in multiple files, earlier files MUST have SMALLER sequence
  //   numbers than later files. This is to ensure LSM invariant since the
  //   earlier files will be placed on a higher level.
  // - `write_global_seqno` must be false (sequence numbers cannot be
  // reassigned).
  //
  // LEVEL PLACEMENT STRATEGY:
  // - Non-overlapping files: All placed at the same level (lowest safe level).
  // - Overlapping files: Separated into batches of non-overlappoing files to
  // maintain LSM invariants.
  //   Earlier files (older, smaller seqno) go to higher levels (higher level
  //   numbers). Later files (newer, larger seqno) go to lower levels (smaller
  //   level numbers).
  //
...
struct IngestExternalFileArg {
  ...
  // TODO
  //
  // EXPERIMENTAL: Ingest entire column family from source
  // Both source_db and source_column_family must be specified together
  DB* source_db = nullptr;
  ColumnFamilyHandle* source_column_family = nullptr;

  // When source_db/source_column_family is specified, how to combine with
  // external_files
  enum class FileCombinationMode {
    kCFThenFiles,  // Ingest CF files first, then external files
    kFilesThenCF,  // Ingest external files first, then CF files
  };
  FileCombinationMode combination_mode = FileCombinationMode::kCFThenFiles;
};
